...
//pot_params[0] = kappas
//pot_params[1] = rs
double re_find_func_nfw_vec(double r, void *p){
...
double bracketing_lambda_t(double f(double r, double params[]), double params[], double out[], double z, double dz){
  double z_min = 0.0;
  double z_max = 0.0;
...
//pot_params[0] = kappas
//pot_params[1] = rs
double r_e_nfw_find(double pot_params[], double *est_err_out, double x_lo, double x_hi, int max_iter, double relative_error, int v){
  ...
  double re = root_find(re_find_func_nfw_vec, params,est_err_out, out[0],out[1],max_iter,relative_error,v);
  return re;
}
