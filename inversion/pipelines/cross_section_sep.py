...
def g_function(u_subs, phi, a_subs, b_subs):
    const1 = (math.cos(phi)**2.0)/( 1.0 - (1.0 - a_subs**2.0 )*u_subs )
    const2 = (math.sin(phi)**2.0)/( 1.0 - (1.0 - b_subs**2.0 )*u_subs )
    #print math.sqrt(u_subs*const1*const2), 'AA'
    return math.sqrt(u_subs*( const1 + const2 ) )
################################################################################
...
def b_argument(u_subs, phi, a_subs, b_subs, n_ind):
    ...
    const3 = math.log( g_function(u_subs, phi, a_subs, b_subs) )
    return const1*const2*const3
################################################################################
def b_function(phi, a_subs, b_subs, n_ind):
    return integrate.quad(b_argument, 0.0, 1.0, \
                          args=(phi, a_subs, b_subs, n_ind))[0]
################################################################################
...
def d_function(phi, a_subs, b_subs, n_ind):
    return integrate.quad(d_argument, 0.0, 1.0, \
                          args=(phi, a_subs, b_subs, n_ind))[0]
################################################################################
...
def const_dist_curve(phi, a_subs, b_subs, kappa_s, r_lambda):
    kd_plus, kd_minus = kappa_dist_const(phi, a_subs, b_subs, kappa_s, r_lambda)
