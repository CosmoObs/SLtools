...
def _tbhdu_from_columns(coldefs):
    return fits.BinTableHDU.from_columns(coldefs)
...
def sort_by_column(tbhdu,fieldname):
    ...
    return _tbhdu_from_columns(coldefs);
...
def merge_tbHDU(tbhdu_A, tbhdu_B):
    ...
    return _tbhdu_from_columns(new_tb);
...
def extend_tbHDU(tbhdu_A, tbhdu_B):
    ...
    new_tb = _tbhdu_from_columns(tbhdu_A.columns,nrows=Nrows);
...
def select_columns(tbhdu, *fieldnames):
    ...
    cols = [];
    for i in inds:
        cols.append( fits.Column(name=coldefs[i].name, format=coldefs[i].format, array=tbdata.field(i)) );
    coldefs = fits.ColDefs(cols);
...
def dict_to_tbHDU(dic, tbname='', *fieldnames):
    ...
    for _arg in args:
        ...
        c.append(fits.Column(name=str(_arg).upper(),format=tipo,array=np.asarray(dic[_arg])));

    newtable = _tbhdu_from_columns(pyfits.ColDefs(c));
