...
for i in range(len(zs)):
  r.append(sqrt(x_rel[i]**2 + y_rel[i]**2))
  phi.append(atan2(y_rel[i],x_rel[i]))
  Eorig.append(-(gamma1[i]*cos(2*phi[i])) - (gamma2[i]*sin(2*phi[i])))
  Borig.append(-(gamma1[i]*sin(2*phi[i])) + (gamma2[i]*cos(2*phi[i])))
...
#------------------Definition of the adimensionalized Hubble function-------------------


def h(z):
 return sqrt(OmM0*(1+z)**3+OmDE0)
...
def integrand(z):
  discrete=[]
  for i in xrange(1, int(z/step)+1):
    discrete.append(c/(H0*gpctocm*h(i*step)))
  return discrete


def Dm(z):
  return  trapz(integrand(z), dx=step)
...
file=open('des_clustersim_reduced_vBruno_out.txt','w')
for i in range(len(zs)):
