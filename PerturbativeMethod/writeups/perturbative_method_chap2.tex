...
(also, we use $ \hat{r}, \hat{\theta}$ in place of \cite{Alard07} $\hat{u}_r,
\hat{u}_\theta$).
...
We now take the first step of perturbing the situation by moving the source off-axis by making $\vec{r}_s$ generally different from zero:
...
and by adding a non-axially symmetric piece of the potential, $\psi$:
...
We do this in both cases to the exact same order $\eps$ by assumption for the moment.
Note that the perturbed distance $y$ is in the source plane.

[Note that this changes the notation of the first eq in \cite{Alard07} eq.~3, which is internally not self-consistent, quite clearly.]

We now write the image position also as a perturbation around the
Einstein Radius, finding that eq's (\ref{eq:rspert}-\ref{eq:potsplit}) will generally modify
eq.(\ref{eq:einsteinradius}) into

\beq
\label{eq:rpert}
r(\theta) = \re +  \eps x(\theta)
\eeq

where the original image point at $\re$ is moved off the circle by a
small distance $\eps x$ (note this changes the form of \cite{Alard07}
eq.~4 slightly, which is somewhat strange notation, since their $dr$
is not necessarily a small distance, and further we do not follow
their convention of setting $\re \equiv 1$).
...
We now do a Taylor series expansion of the potential about the Einstein ring
radius:
...
where $f_n(\theta)$ is a one-dimensional function independent of $r$, and $C_n$ are simply numbers and not functions of either $r$ or $\theta$.
...
so that we reassuringly recover the Einstein ring solution, \eqref{eq:einsteinradius}).
...
which is \cite{Alard07} eq.8 (remember he set $\re \equiv 1$).
...
However, in the perturbative approach, many of the quantities are more
...
Using \eqref{eq:rsexpanded}) and ${\prtl \over \prtl r} ={ 1 \over \epsilon}
{\prtl \over \prtl x}$ (from \eqref{eq:rpert}), the components of the Jacobian matrix $J_{L_P\rightarrow
...
where we have defined the functions $\scriptf$ and $\scriptg$ as
...
\beq
\det \left(J_{L_P\rightarrow S_C}\right)=-\kt\scriptf(\te).
\eeq
...
L_P}\right)=\frac{1}{r}$, as can be verified by eq.(\ref{eq:LctoLp}), we can use eq. (\ref{determinant_mult}) to show that
\beq
J\equiv\det \left(J_{L_C\rightarrow
S_C}\right)=-\frac{\kt\scriptf(\te)}{r}\,.\label{jacobian}
\eeq

Therefore on the critical curves, defined by the condition $J = 0$, using \eqref{eq:scriptf}) we find
...
This is \cite{Alard07} eq.~30, and represents the distortion coordinates of the critical curve from the Einstein circle.  From this last equation, the tangential critical
curve can be expressed in Cartesian coordinates in parametric form as 
\beq
\vec{r}_{\mathrm{crit}}= \left[(\re  +\Delta x_{crit})\cos{\te},(\re+\Delta
x_{crit})\sin{\te}\right],\quad
0 \leq \te < 2\pi.  
\eeq

Now, substituting \eqref{eq:critcurves}) into eqs.~(\ref{y_1}) and
...
Which reduce to \cite{Alard07}'s eqs.~31 for $\re=1$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  From before

Getting \cite{Alard07} eq.33 from the above is straight, and from there we may obtain
% I (MSSG) got most of the way to 
\cite{Alard07} eq.~34 from 33 and 32.
...
\lambda_{-}&=&-\frac{\scriptf(\te)}{r}=-\frac{\scriptf(\te)}{\re+x}\label{
eigenvalue2}
\eea
...
used to define the
cross-section), then we can solve for the radius of the borders of the
deformation cross-section area. The solutions are
\bea
r_{+}(\te)&=&\left|\vec{r}_{\mathrm{crit}}\right|\left(1+\frac{1}{|R_{\rm th}|-1}
\right)\label{rmax}\\
r_{-}(\te)&=&\left|\vec{r}_{\mathrm{crit}}\right|\left(1-\frac{1}{|R_{\rm th}|+1}
\right)\label{rmin}
\eea
...
\bea
\sigma_{dcs} &\equiv&
\int_{\left|\frac{\mu_t}{\mu_r}\right|>|R_{\rm th}|}\!\!\!\!\!\!\!\!\!d^2y=\int_{
\left|\frac{\mu_t}{\mu_r}\right|>C}\!\!\!\!\!\!\!\!\!\left|det\;
\textbf{J}\right| d^2x,\nonumber\\
&=&\int_0^{2\pi}\int_{r_{-}(\te)}^{r_{+}(\te)}\left|-\kappa_2^2\left(\left|\vec{
r}_{\mathrm{crit}}\right|-r\right)\right|\,r{\rm d}r\,{\rm
...
r}_{\mathrm{crit}}\right|^2\,{\rm
d}\theta\label{sec-choq-pert-app2}
\eea

For example, for the SIS model (for which $\kappa_2=1$ and $r_{\rm crit}=R_{\rm E}$) the cross section for deformation arcs is:
\begin{equation}
...
%\gamma &=&   \sqrt \lbracket {1 \over 4}  \lp 2 f_2 - \oorsq \pptsqfz - \oor
%f_1 \rp^2 + \lp \oorsq \pptfz - \twoor \pptfone \rp \rp \rbracket\\
%\eeq
...
Replacing $r_s$ by eq.~(\ref{eq:rsexpanded}), we obtain:
...
So that eq.~(\ref{eq:Rzero}) becomes
...
where $S=1-\eta_s\cos{(2\te)}$. Substituting eq.~(\ref{bar_y1}) and
eq.~(\ref{bar_y2}) into the equation above, we obtain
...
\eea
This is eq.~(28) of Peirani et al. 2008 (recall again, they set $\re=1$ and
$\eta_s=\sqrt{2\eta_0}$)
...
allows us to calculate the analytical expressions for the images
contours coming from lensed sources.
...
elliptical source oriented in any direction with respect to the
semimajor axis of the source potential.  Therefore, we may define a
mean value of the radial coordinate at a given angle $\te$ as
...
is formed. We denote these angles $\te_{i}$ and $\te_{f}$, and we
define the points $P_1$, $P_2$ as the
mean radial vector at these angles, with Cartesian coordinates

\beq
\label{eq:circlecoords}
...
and $P_G$ ($L_{1G}$) and $P_G$ and $P_2$ ($L_{2G}$) (also see Left
Panel in Fig.~\ref{measure_arcs}). Where the Point $P_G$ corresponds to
the barycenter (i.e. gravicenter) of the image, defined by radial
...
or
\beq
\label{eq:singlecirceq}
x^2+y^2+Ax+By+C=0\\
\eeq 

Using eqs.\ref{eq:circlecoords} and \ref{eq:singlecirceq}, and considering the circular arc that passes through three points $P_1$, $P_G$ and
...
and
the outer part). Now, we will use \cite{Alard07} Eq. 12
...
which is exactly \cite{Alard07} eq. 29a, with his additional constant ($C=-\kt\re$).
...
which is our form of \cite{Alard07} eq. 29b.
...
Evaluating at the Einstein radius $r=\re$, we have
...
On the other hand, evaluating Eqs. 19 in \cite{Alard07} at $r=\re$, and using Eq. 16 in
\cite{Alard07} $\phi_0^\prime=1+2C_2(r-\re) -3 C_3(r-\re)^2$
...
Eq. (30) in \cite{Alard07} (to be derived later) is
...
\eeq
we have that for e.g. the NFW profile with ellipticity (as in Eq. 28 of \cite{Alard07})
