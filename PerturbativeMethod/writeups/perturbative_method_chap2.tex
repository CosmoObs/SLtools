...
Following the Pedro's ideas, showed in his Master Degree Thesis, we will give
...
of the arcs (called $W$ and $W_q$). In this report, we will adapt these method
in the framework of the Perturbative Method.
...
and the width
\beq
W(\te)=\dfrac{2}{\kt\,S}\sqrt{SR_{0}^2 -(1-\eta_s^2)\left[ \frac{1}{\re}\frac{\partial \bar{f}_0(\theta)}{\partial \theta}\right]^2}
\label{w_arc}
\eeq
...
defined by
\beq
P_G=(x_{1G},x_{2G})=\left(\frac{1}{N}\sum_{k=1}^N\bar{r}_k\cos{\te_k},\frac{1}{N
} \sum_ { k=1 } ^N\bar { r } _k\sin{\te_k} \right)
\eeq
where $N$ corresponds to the number the point in which the range $[\te_i,\te_f]$
is divided. Note that, the barycenter not necessarily lies inside of the image,
could be lie outside.

Whit the expression above, we have
\bea
L_{1G}&=&\sqrt{x_{1G}^2+x_{2G}^2+\bar{r}^2_i-2\bar{r}_i\left[x_{1G}\cos{\te_i}
+x_{2G}\sin{\te_i} \right]}\\
L_{2G}&=&\sqrt{x_{1G}^2+x_{2G}^2+\bar{r}^2_f-2\bar{r}_f\left[x_{1G}\cos{\te_f}
+x_{2G}\sin{\te_f} \right]}\\
L_2&=&L_{1G}+L_{2G}
\eea


And the third method, corresponds to a more accurate way to measure the
length (corresponding to the length of an arc of circumference  passing
through the points $P_1$ and $P_2$), see dashed line at the Left Panel of the
Fig.~\ref{measure_arcs}, and is given by

\beq
L_3=\int_{\te_i}^{\te_f}\bar{r}(\te)d\te.
\eeq
...
$L_3=\mathcal{L}(\te_f)-\mathcal{L}(\te_i)$.
...
\beq
\te_m=\tan^{-1}{\left[\dfrac{\bar{r}_i\sin{\te_i}+\bar{r}_f\sin{\te_f}}{\bar{r}_i\cos{\te_i}+\bar{r}_f\cos{\te_f}}\right]},
\eeq
...
\eeq
 where $L_k$ corresponds to the measurement of the length of the arcs $k=1,2$ or $3$. 
