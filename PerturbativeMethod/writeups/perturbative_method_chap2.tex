...
of the arcs (called $W$ and $W_q$).  The first method is not properly, for this
in this report we will adapt the second and third methods in the framework of the 
Perturbative Method and also to propose a method similar to Clecio's Method (I forget 
the name) to measure the length.
...
defined by radial vector at the angle $\te_m=(\te_i+\te_f)/2$.

\beq
P_G=(x_{1G},x_{2G})=(\bar{r}(\te_G)\cos{\te_G}+\bar{r}(\te_G)\sin{\te_G})
\eeq
