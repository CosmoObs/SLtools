...
In general, the calculation of theses expression requires numerical method, unless for a few kind of lens model.  For example, for the SIS model, $\mathbf{J}(\vec{x})=1-\frac{1}{x}$ and hence we have

\begin{equation}
\sigma_{dcs}=2\pi\,{\re}^2 \dfrac{|R_{\rm th}|^2+1}{(|R_{\rm th}|^2-1)^2} \label{dcs_sis}
\end{equation}
...
Therefore, the analytical expression for the deformation cross section for the SIEP is
 \begin{equation}
\tilde{\sigma}_{\rm siep}= \dfrac{|R_{\rm th}|^2+1}{(|R_{\rm th}|^2-1)^2} \left[2\pi\left[\mathcal{A}(\varepsilon)+\mathcal{B}(\varepsilon)\right]-2\sqrt{\dfrac{a_{1\varepsilon}}{a_{2\varepsilon}}}\mathcal{B}(\varepsilon)\arctan{\left(2\pi\sqrt{\dfrac{a_{1\varepsilon}}{a_{2\varepsilon}}}\right)} \right]
\end{equation}