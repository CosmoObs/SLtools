...
void print_arcs_exsh(elliptical_source source_in, double pot_params[], double pet_params[], int npts=1000){
...
FILE *outsrc = fopen ("src_plot_exsh.dat" , "w");
    ...
    for(int i=0;i<=npts;i++){
      ...
      if(arg_sqrt(df0dtheta_exsh, source_in, theta, pot_params)>0.0){
      r_p = _r_e+dr_plus(f1_exsh,df0dtheta_exsh,kappa2_nfw(pot_params), source_in, theta, pot_params);
      r_m = _r_e+dr_minus(f1_exsh,df0dtheta_exsh,kappa2_nfw(pot_params), source_in, theta, pot_params);
...
//  Defining the tangential critical radius
     r_tcrit= r_crit(f1_exsh,ddf0dtheta_exsh,kappa2_nfw(pot_params),theta,pot_params); 
...
//  Defining the polar equation of the tangential caustic
     y1_caust=caustic_y1(df0dtheta_exsh,ddf0dtheta_exsh,theta,pot_params);
     y2_caust=caustic_y2(df0dtheta_exsh,ddf0dtheta_exsh,theta,pot_params);
...
// Added by Habib Dumet-Montoya
  ...
  double s_ext_params[]= {0.1,0.02,0.0}; // parametros de perturbação externa com shear
...
//   _r_e=r_e_nfw(pot_params);
  ...
  theta_find(df0dtheta_exsh, source,pot_params );
